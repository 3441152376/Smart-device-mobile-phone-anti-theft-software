...
object FlashLightUtil {
    ...
    private val lightOn = { isSos: Boolean ->
        ...
        if (hasFlashlight()) {
            ...
            Toast.makeText(MyApplication.CONTEXT, "该设备没有闪光灯", Toast.LENGTH_SHORT).show()
    }
    ...
    private val lightOn23 = {
        try {
            val manager = MyApplication.CONTEXT.getSystemService(Context.CAMERA_SERVICE) as CameraManager
    ...
    fun hasFlashlight(): Boolean =
       MyApplication.CONTEXT.packageManager.hasSystemFeature(PackageManager.FEATURE_CAMERA_FLASH)
